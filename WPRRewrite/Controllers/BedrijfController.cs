...
public class BedrijfController(Context context) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Bedrijf>>> GetBedrijven([FromQuery] int? bedrijfId)
    {
        try
        {
            ...
            {
                var bedrijf = await query.FirstOrDefaultAsync(b => b.BedrijfId == bedrijfId);
                if (bedrijf == null)
            ...
            List<AccountZakelijk> bevoegdeMedewerkers = new();
...
public async Task<IActionResult> DeleteBedrijf(int id/*, int kvknummer#1#)
{
    try
    {
